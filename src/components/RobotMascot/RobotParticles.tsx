...
export function RobotParticles({ isDark }: { isDark: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    if (dizzyGroupRef.current && emotion === 'dizzy') {
      ...
      const targetY = action === 'fall' ? 0.3 : 1.2;
      const targetZ = action === 'fall' ? -0.8 : 0;
      dizzyGroupRef.current.position.y = THREE.MathUtils.lerp(dizzyGroupRef.current.position.y, targetY + Math.sin(t * 10) * 0.1, 0.1);
      dizzyGroupRef.current.position.z = THREE.MathUtils.lerp(dizzyGroupRef.current.position.z, targetZ, 0.1);
    }
