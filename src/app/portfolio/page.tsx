...
function ProjectCard({ p }: { p: Project }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full relative aspect-[16/9] lg:aspect-[21/9] overflow-hidden bg-black/5 dark:bg-white/5">
        <img 
          src={p.img} 
          alt={p.title}
          loading="lazy"
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="text-white">
            <div className="flex flex-wrap gap-2 mb-4">
              {p.tags.map((t) => (
                <span key={t} className="text-xs font-bold tracking-wider uppercase px-3 py-1 bg-white/20 backdrop-blur-md rounded-full border border-white/30 text-white shadow-lg">
                  {t}
                </span>
              ))}
            </div>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold font-display tracking-tight mb-2 drop-shadow-xl">{p.title}</h2>
            <h3 className="text-xl md:text-2xl font-medium text-white/90 drop-shadow-md max-w-2xl">{p.hook}</h3>
          </div>
          
          <div className="shrink-0 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 max-w-sm text-white shadow-2xl">
            <h4 className="text-xs uppercase tracking-widest font-bold text-green-400 mb-2">Key Impact</h4>
